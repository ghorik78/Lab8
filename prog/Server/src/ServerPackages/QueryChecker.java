...
public class QueryChecker implements Runnable {
    ...
    public void checkQuery() throws IOException, SQLException {
        if (commandName.equals("register")) manager.registerUser(usrLogin, usrPassword, clientAddress);
        ...
        if (isLegit) {
            if (commandName.equals("add") || commandName.equals("add_if_max")) {
                try {
                    if (commandName.equals("add")) invoker.add(((AddCommand) receivedQuery).getRoute(), commandArgs, usrLogin);
                    else invoker.add_if_max(((AddCommand) receivedQuery).getRoute(), commandArgs, usrLogin);
                ...
                try {
                    ...
                    answerPool.execute(new Responder<>(socket, clientAddress, new Notification("Выполнение этой команды невозможно.", new String[]{"0"}, true)));
                    logger.error("Error during command execution: " + commandName);
                }
            }
        } else {
            answerPool.execute(new Responder<>(socket, clientAddress, new Notification("Отказано в доступе.", null ,false)));
        }
